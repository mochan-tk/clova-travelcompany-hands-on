...
async function handleEvent(event, session) {
  ...
  if (event.type === 'message') {
                ...
                'contents': [
                  {
                    ...
                    'action': {
                      'type': 'uri',
                      'label': 'お支払い',
                      'uri': process.env.LINEPAY_LIFF_URL + '?userid=' + event.source.userId
                    }
                  }
                ]
              }
    ...
    echo = { 'type': 'text', 'text': 'トラベルカンパニーを起動してください。あなたに合ったプランを提案します。' }
    
  } else if(event.type === 'postback'){
    // 埋め込みデータ取得
    const data = event.postback.data.split('-');
    if(data[0] === 'overseas'){
      client.linkRichMenuToUser(event.source.userId, process.env.DOMESTIC_RICHMENU_ID);
      return;
    } else if(data[0] === 'domestic'){
      client.linkRichMenuToUser(event.source.userId, process.env.OVERSEAS_RICHMENU_ID);
      return;
